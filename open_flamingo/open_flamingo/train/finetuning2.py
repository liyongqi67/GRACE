...
def main():
    ...
    parser.add_argument(
        "--unfreeze_all",
        action="store_true",
        help="if True, we unfreeze all parameters.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--wandb_project",
        default="Gen_Cross_Modal-Retrieval",
        type=str,
    )
    parser.add_argument(
        "--wandb_entity",
    ...
    parser.add_argument(
        "--training_strategy", default="hybrid", type=str, choices=["full", "i2id", "t2id"]
    )
    parser.add_argument(
        "--pretrained_itid",
        type=str,
        default=None,
        help="the path to the checkpoint after i2id training",
    )
    parser.add_argument(
        "--lora",
        default=False,
    ...
    if args.pretrained_checkpoint is not None:
        if ".pt" in args.pretrained_checkpoint:
            ...
            checkpoint_path = hf_hub_download(args.pretrained_checkpoint, "checkpoint.pt")
    ...
    if args.pretrained_itid is not None:
        checkpoint = torch.load(args.pretrained_itid,map_location="cpu")
        checkpoint = checkpoint["model_state_dict"]
        checkpoint = {k.replace("module.", ""): v for k, v in checkpoint.items()}
        model.load_state_dict(checkpoint, strict=False)
    ...
    if args.fsdp:
        ...
        # model = FSDP(model, process_group=process_group, sharding_strategy=ShardingStrategy.FULL_SHARD, auto_wrap_policy=None, backward_prefetch=BackwardPrefetch.BACKWARD_PRE, mixed_precision=mp_policy, ignored_modules=None, param_init_fn=None, device_id=device_id, sync_module_states=True, forward_prefetch=True, limit_all_gathers=True, use_orig_params=args.fsdp_use_orig_params)        
        model.wrap_fsdp(wrapper_kwargs, device_id, args.unfreeze_all)
        ddp_model = model
    ...
    if args.rank == 0:
        print('len(params_to_optimize)', len(params_to_optimize))
        for name in params_to_optimize:
            print(name[0],name[1].requires_grad)
    if not args.fsdp or args.fsdp_use_orig_params:

        if args.unfreeze_all: ##########yongqi add this loop for training all paramaters
    ...
    laion_dataset = get_data(args, image_processor, tokenizer, "text2id")
    ...
    for epoch in range(resume_from_epoch, args.num_epochs):
        ...
        if args.training_strategy == "hybrid":
            train_one_epoch3(
                args=args,
                model=ddp_model,
                epoch=epoch,
        ...
        elif args.training_strategy == "i2id":
            train_one_epoch2(
                args=args,
                model=ddp_model,
                epoch=epoch,
        ...
        elif args.training_strategy == "t2id":
            train_one_epoch4(
                args=args,
                model=ddp_model,
                epoch=epoch,
