...
def main():
    ...
    parser.add_argument(
        "--i2id_textsim_threshold",
        default=0.0,
        type=float,
        help="threshold for filtering images in i2id based on image-text similarity",
    )
    parser.add_argument(
        "--i2id_max_num_images",
        default=1,
        type=int,
        help="max number of images per sequence in i2id / chatgpt",
    )
    parser.add_argument(
        "--i2id_min_num_images",
        default=1,
        type=int,
        help="min number of images per sequence in i2id / chatgpt",
    )
    parser.add_argument(
        "--t2id_textsim_threshold",
        default=0.0,
        type=float,
        help="threshold for filtering images in t2id based on image-text similarity",
    )
    parser.add_argument(
        "--t2id_max_num_images",
        default=1,
        type=int,
        help="max number of images per sequence in t2id / chatgpt",
    )
    parser.add_argument(
        "--t2id_min_num_images",
        default=1,
        type=int,
        help="min number of images per sequence in t2id / chatgpt",
    )
    parser.add_argument(
        "--id2caption_textsim_threshold",
        default=0.0,
        type=float,
        help="threshold for filtering images in id2caption based on image-text similarity",
    )
    parser.add_argument(
        "--id2caption_max_num_images",
        default=1,
        type=int,
        help="max number of images per sequence in id2caption / chatgpt",
    )
    parser.add_argument(
        "--id2caption_min_num_images",
        default=1,
        type=int,
        help="min number of images per sequence in id2caption / chatgpt",
    )
...
##############the follwoing is added by yongqi for autoregressive image retrieval
    parser.add_argument(
        "--add_extra_id_tokens",
        type=str,
        default=None,
    ...
    parser.add_argument(
        ...
        type=str,
        help="path to checkpoint that afer large scale pretrianing",
        default=None,
    )
    parser.add_argument(
        "--new_class_embed",
        default=False,
        action="store_true",
        help="create a new class embem in flamingo",
    )
...
##########################################################
    ...
    if args.pretrained_checkpoint is not None:
        if ".pt" in args.pretrained_checkpoint:
            ...
            checkpoint = hf_hub_download(args.pretrained_checkpoint, "checkpoint.pt")
            checkpoint = torch.load(checkpoint, map_location="cpu")
            checkpoint = {k.replace("_checkpoint_wrapped_module.", "").replace("lang_encoder", "lang_model").replace("perceiver", "vision_tokenizer"): v for k, v in checkpoint.items()}
            model.load_state_dict(checkpoint, strict=False)

    if args.add_extra_id_tokens !=None :
        import pickle
        with open(args.add_extra_id_tokens, 'rb') as f:
        ...
        for key in add_extra_id_tokens:
            if "-" in key:
                ...
                id_token_dict[key]=1
        if args.rank == 0:
            print('the list of new added tokens', list(id_token_dict.keys()))
        # tokenizer.add_tokens(list(id_token_dict.keys()))
        # model.lang_model.resize_token_embeddings(len(tokenizer))
        # #######/home/share/yongqi/project/AutoregressiveImageRetrieval/data/Openflamingo_format/image_emb.npy
        # import numpy as np
        # image_emb = np.load('../AutoregressiveImageRetrieval/code/data_process/image_emb.npy')
        # print(image_emb.shape)
        # print(model.lang_model.transformer.wte.weight.shape)
        # with torch.no_grad():
        #     updated_weight = model.lang_model.transformer.wte.weight.clone()
        #     updated_weight[-image_emb.shape[0]:,] = torch.from_numpy(image_emb)
        #     model.lang_model.transformer.wte.weight = torch.nn.Parameter(updated_weight)
    ...
    total_training_steps = (
        ...
        // (getattr(args, f"batch_size_{datasets_to_train_on[0]}")*args.world_size)           ##########yongqi add the args.world_size to fix the bug
    ) * args.num_epochs
