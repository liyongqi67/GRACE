...
def train_one_epoch2(
    ...
    for step_num, batches in tqdm(
        ...
        losses_to_log = {}
        batch_metadata_to_log = {}
        for dataset_ix, (images, (input_ids, attention_mask), labels) in enumerate(batches):
...
def save_checkpoint(model, optimizer, lr_scheduler, epoch, args):
    ...
    if args.fsdp:
        ...
        # optim_state = FSDP.optim_state_dict(model, optimizer) #yongqi commit to save all paramaters
    else:
        model_state = model.state_dict()
        # optim_state = optimizer.state_dict() #yongqi commit to save all paramaters

    if args.rank == 0:
        model_state = filter_state_dict_to_trainable(model, model_state)  #yongqi commit to save all paramaters
        ...
        checkpoint_dict = {
            ...
            # "optimizer_state_dict": optim_state, #yongqi commit to save all paramaters
...
def save_deepspeed_checkpoint(model, epoch, args):
    ...
    model.save_checkpoint(
        ...
        save_latest=True,
        tag=f"epoch_{epoch}",
        exclude_frozen_parameters=False, # Save all parameters
    )
